...
edad_limit = StringVar()
email_limit = StringVar()

# Crear los campos de entrada para los datos del alumno
Label(ventana, text="Id:").grid(row=0, column=0, padx=5, pady=5)
entrada_id = Entry(ventana, textvariable=id_limit)
entrada_id.grid(row=0, column=1, padx=5, pady=5)
entrada_id.bind("<Return>", enter1)

Label(ventana, text="Nombre:").grid(row=1, column=0, padx=5, pady=5)
entrada_nombre = Entry(ventana, textvariable=nombre_limit)
entrada_nombre.grid(row=1, column=1, padx=5, pady=5)
entrada_nombre.bind("<Return>", enter2)

Label(ventana, text="Edad:").grid(row=2, column=0, padx=5, pady=5)
entrada_edad = Entry(ventana, textvariable=edad_limit)
entrada_edad.grid(row=2, column=1, padx=5, pady=5)
entrada_edad.bind("<Return>", enter3)

Label(ventana, text="Email:").grid(row=3, column=0, padx=5, pady=5)
entrada_email = Entry(ventana, textvariable=email_limit)
entrada_email.grid(row=3, column=1, padx=5, pady=5)

#! LIMITE DE ENTRADA
id_limit.trace("w", lambda *args: character_limit(id_limit))
nombre_limit.trace("w", lambda *args: character_limit(nombre_limit))
edad_limit.trace("w", lambda *args: character_limit(edad_limit))
email_limit.trace("w", lambda *args: character_limit2(email_limit))
